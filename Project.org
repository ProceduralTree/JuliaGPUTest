...
* Introduction
This project thesis builds upon the work in our bachelor thesis, by introducing a simple boudary condition approach to a variation of the solver useed therein. In Chapter [[Fundamentals]] we introduce the Cahn Hilliard Equation in the fomulation that we use for this project. This project used a two dimensional second order version of  this CH equation rather than the usual 1D 4th order one, to simplify the numerical implementation. The solver itself biulds upon a finite difference discretization of this equation. In Chapter [[Numerical Solver]], together with the discrete domains, on wich we run our numerical solver ,we introduce a jacoby iteration to solve the linear system derrived from the afforementioned discretization.  The numerical solver in this thesis is gpu accelerated, and the discretization we chose to base our solver on is capable to calculate on all domains as long as a characteristic function is given. Therefore, we introduce two domains, on which we present our findings. The primary goal of this work is then the boundary condition approach in Chapter [[Boundary adaptation]]. Conceptionally the boundary condition we introduce simply consists of a constant value added in the linear system to all equations corresponding to gridcells on the boundary. The actual implementation is capable of doing this for arbitrary domains. We present the reults of this method on two Domains, a square one in [[Numerical evaluation]] and a circular domain in [[Numerical evaluation on a circle]]
* Fundamentals
...
To show the behavior of the CH sover in [[fig:angle0c]], we first emplay no-flux boundary conditioons on a circular domain. We observe the interface perpendicular on the boundary, as we expect.
...
The results we oserve in [[fig:angle1c]] are similar to the reults on a square domain in [[fig:angle1]]. The contact angle is 18° i.e. the interface does not touch the boundary and runs parallel to it.
#+name: fig:angle1c
...
The results for \(C=-1\) in [[fig:angle-1c]] on the circular domain, are similar to the results in [[fig:angle-1]]  onth esqaure domain as well ,where the interface touches the boundary and iun parallel with a contact angle of 0°.
#+name: fig:angle-1c
...
#+RESULTS:
[[file:/tmp/babel-miHWkx/julia-h4yghb.png]]

* Energy and mass behaviour :noexport:
The Chan Hilliard equation was originally derrived from a Ginzburg-Landau Energy [cite:@Wu_2022]. This energy gives a meassure of total curvature, and is prooven to decrease in time for no-flux boundary condition for no-flux boundary conditions. The energy is given as
\begin{equation}
...
include("src/util.jl")
arr = _init()
s,f = solve_and_evaluate(arr , n, bulk_energy ,  θ=0.5f0)
plot(f , yaxis=:log)
#+end_src
