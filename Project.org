...
\end{aligned}
\end{equation}
this aproach provides a semi implicit time discretization were linear terms are evaluated implicitly and the nonlinear double well potential is evaluated explicitly.
...
We observe the most interesting behavior for values between \((-1,1)\), where we observe the contact angle of the interface on the boundary to change from parallel 0° to parallel 180°.
...
#+end_src
* angle
In previous experiments we noted that the angle of the interface changes with diferent input parameters. While we do not have a mathematical derivation of this relation, we aim to provide numerical insight in this chapter. We calculate this angle using the gradient of the phase-field \(\nabla \phi_{ij}\) and the normal of our domains' boundary.
\begin{align}
\label{eq:1}
